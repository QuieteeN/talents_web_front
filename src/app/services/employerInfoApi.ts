...
export const employerInfoApi: any = createApi({
    reducerPath: "employerInfoApi",
    baseQuery: fetchBaseQuery({ 
        baseUrl: 'http://localhost:5000/api/employerInfo',
        credentials: 'include'
    }),
    endpoints: (builder) => ({
        ...
        updateMainInfo: builder.mutation<void, { companyName: string, companyDescription: string }>({
            query: (body: { companyName: string, companyDescription: string }) => ({
                url: '/update-main-info',
                method: 'PUT',
                body,
            }),
        }),
        updateContactInfo: builder.mutation<void, { name: string, surname: string, email: string, phoneNumber: string }>({
            query: (body: {name: string, surname: string, email: string, phoneNumber: string}) => ({
