...
import { useGetEmployerProfileByIdQuery } from "../../../../app/services/profileApi";
...
const ProfileContent: React.FC<{ employerId: number }> = ({ employerId }: { employerId: number }) => {
    const [companyData, setCompanyData] = useState<any | null>(null);
    const getEmployerProfile = useGetEmployerProfileByIdQuery({ employerId: employerId });
        ...
        <div className={classes.company_content}>
            {getEmployerProfile.isLoading ? <ReactLoading type='spinningBubbles' color='#3B26B6' height={'20%'} width={'20%'} /> : <>
                <div className={classes.company_header}>
                    ...
                    <h2>{companyData?.employer?.info?.companyName || `${companyData?.employer?.name} ${companyData?.employer?.surname}`}</h2>
                </div>
                <div className={classes.company_description}>
                    <pre>{companyData?.employer?.info?.companyDescription || `Без описания`}</pre>
                </div>
                        ...
                        <div className={classes.data_item}>
                            ...
                            <span className={`${classes.data_content} ${classes.socials_content}`}>
                                {companyData?.employer?.info?.socials.filter((social: any) => social.type !== 'official').map((social: any) => <>
                                    <a href={social.url} title={social.type}>
